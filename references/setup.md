...
Эти инструменты в нашем тесте дали более быструю транскрипцию при сопоставимом качестве финального meeting-отчёта. **Не интегрированы** в `transcribe.py` — используются как самостоятельные CLI; после получения `.txt`/`.srt` агент читает результат и пишет отчёт по `report_template.md` как обычно.
