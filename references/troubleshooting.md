...
## Один из двух параллельных ASR-процессов упал
...
При нехватке общей GPU-памяти закрыть ресурсоёмкие приложения либо оставить
процессы параллельными, но вынести GigaAM на CPU: `--gigaam-device cpu`.
Скрипт намеренно не переключает модель и не переиспользует старый артефакт
молча.
