...
4. Определи платформу (uname -sm) и **поставь рекомендуемый скиллом
   ...
   **Важный нюанс про качество — две разные оси:**
   - **Выбор бэкенда** (whisper.cpp / mlx-whisper / faster-whisper) — все три
     гоняют один и тот же чекпоинт OpenAI Whisper. WER между ними в пределах
     1–2%, на слух не услышишь. Выбор по скорости и удобству установки.
   - **Выбор модели** (`large-v3-turbo` vs `large-v3`) — это **разные
     чекпоинты**. Turbo — дистиллят, в ~2× быстрее. На чистом аудио разница
     умеренная, на сложном (тихая речь, длинные паузы, перекрытия) `large-v3`
     заметно точнее и устойчивее к залипаниям. Не «1–2%».
   ...
   **Затем спроси меня, какую модель скачать (одну, не обе)** — и кратко
   объясни выбор:

   - **`large-v3-turbo`** (~1.5 ГБ) — пресеты `fast`/`balanced`. Дефолт для
     рабочих созвонов и большинства записей.
   - **`large-v3`** (~3 ГБ) — пресет `quality`. Бери сразу, если планируешь
     транскрибировать важные/сложные записи (юридически важное, плохое аудио,
     интервью на публикацию, точные дословные цитаты).

   Не качай обе по умолчанию — это лишние ~3 ГБ. Команда:
       cd ~/whisper.cpp && bash models/download-ggml-model.sh <выбранная-модель>

   Вторую модель всегда можно докачать той же командой позже — скилл сам
   подхватит. `setup_check.py` после установки покажет, какие пресеты доступны
   и как добавить недостающее.
