...
2. для русской речи прогонит полные GigaAM и Whisper `large-v3` (`quality`, beam=5),
...
- **Двойная транскрипция русской речи** через локальные GigaAM + Whisper на всей записи: два полных независимых прохода, по умолчанию по очереди.
...
- **Проверка обновлений** после готового отчёта — раз в сутки, с перечнем изменений; обновление только по явному согласию, выключается одной переменной окружения.

### Штатное поведение

| Компонент | Поведение |
|---|---|
| Русская речь | GigaAM + Whisper на всей записи (по очереди; `--parallel` — одновременно), затем LLM-сверка и подтверждение человеком |
...
| Проверка обновлений | Включена; после отчёта, не чаще раза в сутки; `MEETING_TRANSCRIBE_NO_UPDATE_CHECK=1` выключает |
...
2. запустит GigaAM и Whisper `large-v3` (`quality`, beam=5) на всей записи,
...
│   ├── dual_transcribe.py     # запуск обоих движков и сверка
...
│   ├── check_update.py        # проверка обновлений после отчёта
│   └── requirements.txt
├── assets/
...
├── VERSION                    # версия установленной копии
├── CHANGELOG.md               # история изменений, исходник текста релиза
└── tests/                     # python3 -m unittest discover -s tests
```

## Обновления

После того как отчёт сохранён (и только после — до транскрипции проверка не запускается), скилл смотрит, не вышла ли новая версия, и, если вышла, показывает её номер, список изменений и ссылку на релиз. Обновление он предлагает, но не выполняет: `git pull` в каталоге скилла делается только после явного «да», отдельной командой.

**Куда и когда уходит запрос.** Один анонимный GET к публичному API релизов GitHub — `https://api.github.com/repos/gman-dev-nov/meeting-transcribe-skill/releases/latest` — не чаще раза в сутки. Ни токенов, ни идентификаторов установки, ни телеметрии, ни данных о записи не передаётся: тело запроса пустое, отправляется только сам факт запроса. Результат кешируется в `~/.cache/meeting-transcribe/update-check.json` (или в `$XDG_CACHE_HOME`), и до истечения суток проверка не ходит в сеть вообще. Отсутствие сети, таймаут и лимит анонимных запросов GitHub (60 в час) не печатают ни строчки: проверка обновлений не имеет права испортить готовый отчёт.

**Выключить полностью:**

```bash
export MEETING_TRANSCRIBE_NO_UPDATE_CHECK=1
```

С этой переменной скилл не ходит в сеть никогда — ASR-часть и так работает локально.

**Обновление вручную:**

```bash
python3 ~/.claude/skills/meeting-transcribe/scripts/check_update.py --update
```

Команда отказывается работать и ничего не трогает, если в каталоге скилла есть незакоммиченные правки, если он не под git (например, распакованная копия — тогда обновление только заменой каталога) или если скилл установлен плагином — плагины обновляет сам Claude Code. Новая версия начинает действовать со следующей сессии: текущая уже держит `SKILL.md` в контексте.

Диагностика без обращения к сети — `python3 scripts/check_update.py --status`.
...
ASR-скрипты сами ничего не отправляют в облако. Единственный исходящий запрос самого скилла — суточная проверка обновлений после отчёта (см. «Обновления»), она анонимна и выключается `MEETING_TRANSCRIBE_NO_UPDATE_CHECK=1`. Модели скачиваются при установке/первом запуске и затем работают локально. Тексты транскриптов обрабатываются тем Claude/ChatGPT/Codex-интерфейсом, в котором запущен скилл; его политика данных применяется и к LLM-сверке.
