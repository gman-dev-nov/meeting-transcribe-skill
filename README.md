...
Локальный скилл для **Claude Code** и **OpenAI Codex CLI**: расшифровка созвонов и встреч + автоматический отчёт с TL;DR, решениями, action items и цитатами. Работает полностью офлайн на Mac (Apple Silicon), Linux и Windows. Заточен под русский язык.

> Скилл универсальный — `SKILL.md` имеет общий формат, который оба CLI читают одинаково. Различается только путь установки: `~/.claude/skills/` для Claude Code и `~/.agents/skills/` для Codex.

## Как пользоваться

Когда скилл установлен — просто упомяни путь к записи агенту:
...
## Ленивая установка через Claude Code

Если не хочется ставить руками — открой Claude Code в любой папке и скопипасти этот промпт. Агент сам всё проверит, поставит и спросит только то, что важно.

> Для **OpenAI Codex CLI** используй тот же промпт, заменив `~/.claude/skills/` на `~/.agents/skills/`. Остальное (venv, бэкенды, диаризация, setup_check) — без изменений.
...
5. Спроси, нужна ли диаризация (метки спикеров — кто что сказал). Цена:
   ...
   - **Когда стоит включать:** если запись длиннее 2ч и спикеров 6+, или нужен
     ...
     цитат. Иначе агент атрибутирует реплики по контексту сам.
...
После этого скилл готов: брось любой `.mp4`/`.m4a` агенту со словами «транскрибируй» — он сам всё сделает.
...
git clone https://github.com/gman-dev-nov/meeting-transcribe-skill.git ~/.agents/skills/meeting-transcribe
```

Если используешь оба CLI — клонируй один раз и сделай симлинк во вторую локацию (см. `CONTRIBUTING.md`).
...
После установки скилл работает сразу — Claude Code автоматически подхватывает скиллы из `~/.claude/skills/`, OpenAI Codex CLI — из `~/.agents/skills/`. Базовый сценарий и список явных уточнений — в разделе [«Как пользоваться»](#как-пользоваться) выше.
...
## Запуск из командной строки (без агента)
...
```text
meeting-transcribe/
├── SKILL.md                   # инструкции для агента (Claude Code и Codex читают их сами)
├── agents/
│   └── openai.yaml            # метаданные для Codex (опционально, Claude Code игнорирует)
├── scripts/
