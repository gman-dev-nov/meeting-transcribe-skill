...
## Ленивое обновление через Claude

Копии, поставленные до версии 1.0.0, — это обычная папка с файлами без git: обновить её на месте нельзя, нужно заменить целиком. Начиная с 1.0.0 скилл делает это сам (см. [Обновления](#обновления)), но один раз перейти на клон придётся руками. Скопипасть Claude Code этот промпт:

```text
Обнови у меня скилл meeting-transcribe до последней версии
(https://github.com/gman-dev-nov/meeting-transcribe-skill).

У меня стоит старая версия: обычная папка с файлами, скачанная вручную,
без git. Обновить её на месте нельзя, нужно заменить копию целиком.
Сделай это аккуратно, по шагам, и на каждом шаге показывай, что получилось.

1. Найди установленную копию. Обычное место — ~/.claude/skills/meeting-transcribe.
   Если её там нет — поищи папку с файлом SKILL.md, где в первой строке
   name: meeting-transcribe, и скажи мне, что нашёл, прежде чем трогать.

2. Проверь, не правил ли я там что-нибудь. Особенно
   scripts/lexicons/terms.json — туда добавляют свои термины и написания имён.
   Если найдёшь непустые следы правок, покажи их мне и дождись ответа,
   прежде чем продолжать: их надо будет перенести в новую копию руками.

3. НИЧЕГО НЕ УДАЛЯЙ. Старую папку перенеси в ~/.claude/skills-backup/
   с датой в имени. Важно: бэкап не должен остаться внутри ~/.claude/skills —
   папка оттуда подхватывается как ещё один скилл с той же командой,
   и потом непонятно, какая из копий работает.

4. Поставь свежую копию клоном по HTTPS:
   git clone https://github.com/gman-dev-nov/meeting-transcribe-skill.git \
     ~/.claude/skills/meeting-transcribe
   Именно HTTPS: репозиторий публичный, аккаунт на GitHub и SSH-ключ не нужны.

5. Проверь, что всё встало:
   - cat ~/.claude/skills/meeting-transcribe/VERSION — должна быть 1.0.1 или новее;
   - cd в эту папку и python3 -m unittest discover -s tests — все тесты зелёные;
   - python3 scripts/check_update.py --status — в JSON должно быть
     "install_kind": "git", это значит, что дальше обновления пойдут одной командой.

6. Скажи мне: какая версия встала, куда положен бэкап и что в нём осталось.
   Скажи также, что новая версия заработает со следующего запуска Claude Code:
   инструкции старой уже загружены в текущую сессию.

Модели, venv'ы и ffmpeg трогать не надо — они лежат вне папки скилла
и переустановки не требуют.
```

Модели, venv и ffmpeg лежат вне папки скилла, поэтому переустановки не требуют — меняются только файлы самого скилла.
