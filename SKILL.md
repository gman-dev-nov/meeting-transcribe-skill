...
4. Агент сам пишет отчёт по шаблону `assets/report_template.md` на основе транскрипта.

> **Эмпирически:** разница в финальном отчёте между `fast` и `quality` обычно пренебрежимо мала — модель восстанавливает структуру даже из шумного транскрипта. Бери `fast` по умолчанию, `quality` — только когда нужны точные дословные цитаты.
...
На типовом созвоне 4–6 спикеров агент атрибутирует реплики по контексту самостоятельно. Диаризация добавляет ~10–20% точности и улучшает читаемость, но стоит времени.
