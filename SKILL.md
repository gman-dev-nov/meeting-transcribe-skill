...
> **Эмпирически:** разница в финальном отчёте между `fast` и `quality` обычно пренебрежимо мала — Claude восстанавливает структуру даже из шумного транскрипта. Бери `fast` по умолчанию, `quality` — только когда нужны точные дословные цитаты.
